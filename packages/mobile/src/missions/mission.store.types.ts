...
//
// ARCH-STORE-00 : ce fichier fixe la FORME de l'état et le CONTRAT des actions.
// Aucune logique (reducers, appels API, retry, refetch) ici — l'implémentation
// est dans mission.store.ts (ARCH-STORE-01).
//
// Décisions justifiées dans docs/adr/store-architecture.md :
 ...
 * au niveau collection (fraîcheur du dernier `GET /missions`). Cf. ADR Décision 1.
 */
 ...
 * — défini ici pour ne pas re-typer le jour où elle arrive.
 */
...
/**
 * Surface d'API du slice. Le temps est injecté (`at: number`) — jamais `Date.now()`
 * interne (déterminisme / testabilité). Implémentation : mission.store.ts.
 */
...
// ── Compat UX-OFFLINE-04 (forme simplifiée, consommée par le store offline) ──

/**
 * Forme simplifiée pour la persistance offline (branch UX-OFFLINE-04).
 * Utilise un tableau plat + lastSyncedAt (pas de normalisation par id).
 * À terme, remplacée par `PersistedMissionCache` lors de la convergence des branches.
 */
...
/**
 * Interface du store simplifié (UX-OFFLINE-04) — consommée par le store offline
 * actuel (`mission.store.ts`). À terme, les composants consommeront `MissionSlice`.
 */
