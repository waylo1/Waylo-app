...
**Moteur de pénalité 120/200 — arbitrage de fraude (Sprint 14)** : `POST /api/admin/missions/:id/arbitrate-fraud` ([`src/admin/arbitrage.route.ts`](../src/admin/arbitrage.route.ts), nouveau plugin monté sous `/api/admin`) matérialise le moteur reporté en S13, avec la base de calcul fixée : **(budget [Valeur Objet] + commission [Frais Service])**. Réservé aux admins (`isRequestAdmin`, exporté de `mission.route.ts` — source unique). Sur une mission `DISPUTED`, **une seule `$transaction` sans aucun appel Stripe** : transition anti-TOCTOU `DISPUTED → DISPUTED_FRAUD` (gelé terminal) + `PenaltyDebitOutbox` PENDING (ponction voyageur = base × 2 = **200%**) + `LedgerEntry` `FRAUD_PENALTY_COLLECTED` (200%) & `BUYER_REFUND_COMPENSATION` (base × 1,2 = **120%**) + `AdminAuditLog` (D-c). Marge plateforme **80%** implicite (200 − 120). Idempotence : la transition unique + `PenaltyDebitOutbox.missionId @unique` garantissent une seule ponction (double appel → 400 `MISSION_NOT_DISPUTED`, sans doublon). **Décisions (vs spec littérale)** : (1) `LedgerType` réels = `FRAUD_PENALTY_COLLECTED`/`BUYER_REFUND_COMPENSATION` (noms du spec), stockés en **magnitudes positives** (convention `amountCents ≥ 0`, le type porte la direction « +200% / −120% »), **exclus des invariants A/B/C** (cf. §3) — aucune corruption de l'escrow ; (2) ajout de `MissionStatus.DISPUTED_FRAUD` (implicite dans le spec) ; (3) `PenaltyDebitOutbox` enrichi des champs spec (id, userId, amountCents, status, createdAt) + `missionId @unique` (idempotence) + `onDelete: Cascade` (isolation tests, miroir `Review`) + `updatedAt`/relations/index (cycle outbox du futur worker) ; (4) `AdminAuditLog` ajouté (invariant D-c — toute décision admin est tracée) ; (5) **aucun mouvement d'argent dans ce sprint** : la route journalise l'intention (outbox) + le ledger et gèle la mission ; l'EXÉCUTION Stripe (débit carte voyageur, sortie du hold acheteur) relève du **penalty-worker (Sprint 15, ci-dessous)** — au moment de l'arbitrage l'escrow reste `HELD`, règle d'or trivialement respectée. **Migration** `20260617140000_add_fraud_penalty_engine` (enum `ADD VALUE` ×3 + table `PenaltyDebitOutbox`).

**Exécution de la ponction — penalty-worker (Sprint 15)** : [`src/workers/penalty.worker.ts`](../src/workers/penalty.worker.ts) matérialise l'intention `PenaltyDebitOutbox` posée en S14 — **SEUL chemin** qui débite la carte de garantie du voyageur. Pattern outbox, **miroir strict de transfer-worker** (claim `FOR UPDATE SKIP LOCKED` → `SUBMITTED` ; backoff exponentiel `2^attempts` ; abandon à M=5). Séquence (règle d'or — **aucun appel Stripe en transaction**) : (1) HORS tx, charge **off-session** 200% de `User.stripePaymentMethodId` (`paymentIntents.create` `confirm:true, off_session:true`, `customer` si présent, clé déterministe `penalty_debit_<outboxId>`) ; statut `≠ 'succeeded'` → échec (jamais de faux SETTLED) ; (2) **sur succès uniquement** HORS tx, `paymentIntents.cancel` du hold acheteur HELD (`penalty_release_<missionId>`, jamais capturé — la mission `DISPUTED_FRAUD` est gelée, aucune course) ; (3) `$transaction` : escrow `HELD → CANCELLED` (`updateMany` conditionnel anti-TOCTOU) + outbox `SETTLED` + `stripePaymentIntentId`. **Échec** (carte refusée/fermée, off-session non abouti) → `FAILED` + `lastError` + backoff ; au M-ième → `ABANDONED` + alerte **critique** `PENALTY_DEBIT_ABANDONED` (une seule fois). **Décision d'archi** : la libération du hold acheteur est **conditionnée au recouvrement** (charge voyageur réussie) — un abandon laisse l'escrow `HELD` (créance ouverte + hold non libéré = double action humaine, signalée dans l'alerte). Démarré dans `server.ts` (cadence transfer-worker, `clearInterval` au SIGTERM). **Migration** `20260617150000_penalty_outbox_retry_fields` : ajoute `attempts`/`lastError`/`stripePaymentIntentId @unique` à `PenaltyDebitOutbox` (nécessaires au backoff/abandon). Nouveau code d'alerte `PENALTY_DEBIT_ABANDONED` (critical) dans [`alerts.ts`](../src/alerts.ts).
...
2. **`idempotencyKey` Stripe déterministes** (persistées, jamais recalculées) :
   ...
   | `penalty_debit_<outboxId>` | `paymentIntents.create` (charge off-session 200%) | **penalty-worker** (S15) — débit carte garantie voyageur ; clé dérivée de l'id outbox immuable |
   | `penalty_release_<missionId>` | `paymentIntents.cancel` (libération hold) | **penalty-worker** (S15) — annule le hold acheteur HELD sur ponction réussie |
...
3. **Contraintes `@unique` DB** : `EscrowTransaction.{missionId, stripePaymentIntentId,
   ...
   `PenaltyDebitOutbox.{missionId (une seule ponction de fraude par mission), stripePaymentIntentId (un seul débit réussi)}` ;
...
- **penalty-worker** (~1 min, [`penalty.worker.ts`](../src/workers/penalty.worker.ts), Sprint 15) :
  exécute la ponction de fraude posée par `/arbitrate-fraud`. Même éligibilité/backoff/abandon que
  transfer-worker (claim `FOR UPDATE SKIP LOCKED` → `SUBMITTED`). HORS tx : (1) charge **off-session**
  200% de la carte de garantie voyageur (`confirm:true, off_session:true`, `penalty_debit_<id>`) ;
  (2) sur succès UNIQUEMENT → `paymentIntents.cancel` du hold acheteur HELD (`penalty_release_<missionId>`,
  jamais capturé — miroir `resolve-refund`). Puis `$transaction` : escrow `HELD → CANCELLED` (conditionnel
  anti-TOCTOU) + outbox `SETTLED` + `stripePaymentIntentId`. Échec carte → `FAILED` (backoff) ; M=5 →
  `ABANDONED` + alerte **critique** `PENALTY_DEBIT_ABANDONED` (créance ouverte **+ hold acheteur non libéré** :
  la libération est conditionnée au recouvrement). L'escrow acheteur n'est touché que sur ponction réussie.
